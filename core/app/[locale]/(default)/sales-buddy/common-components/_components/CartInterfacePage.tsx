...
export default function CartInterface() {
  ...
  const [comment, setComment] = useState<string | null>(null); // Comment state
  ...
  const handleCommentSubmit = async (e: React.FormEvent) => {
    ...
    const createCommentData = {
      comment: commentRefs.comment.current?.value?.trim() || '',
    };
  ...
  const toggleAccordion = (index: number) => {
   ...
   // Flag to check if comment is saved

  // Handle the save comment functionality
  // const handleSaveComment = () => {
  //   setIsCommentSaved(true);
  //   setIsCommentVisible(false); // Hide form after saving comment
  // };

  // Handle edit comment functionality
  const handleEditComment = () => {
    setIsCommentVisible(false); // Show the form again to edit comment
  };

  // Handle delete comment functionality
  const handleDeleteComment = () => {
    setComment(null); // Clear the comment
    ...
    return fields.map((item, component) => (
      <div key={item.id} className="space-y-[10px]">
        <div className="flex flex-col">
          ...
          {!component && <SelectDropdown id="supplier" value={formData.supplier} onChange={(e) => setFormData({ ...formData, supplier: e.target.value })} />}
        </div>
      </div>
    ));
  };
  ...
  const accordions = [
    ...
    // {
    //   title: <AccordionTitle icon={ChatIcon} text="Add Order Comments (Internal)" onClick={() => toggleAccordion(2)} />,
    //   content: (
    ...
    //                 onClick={() => setComment(comment)} // Re-enable comment editing
    //               >
    ...
    //               <Button
    //                 className="item-center flex justify-center p-1"
    //                 onClick={() => setComment(null)} // Clear the comment
    //               >
